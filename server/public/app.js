...
/** Which slice of history the console is showing: all, demo, or real. */
let dataMode = sessionStorage.getItem('asktrix.admin.dataMode') || 'all';
...
/** Appends the current data-mode filter to a console request. */
function withMode(path) {
 if (dataMode === 'all') return path;
 return path + (path.includes('?') ? '&' : '?') + 'data=' + dataMode;
}
...
document.querySelectorAll('#data-mode .seg__btn').forEach((button) => {
 button.addEventListener('click', () => {
 document.querySelectorAll('#data-mode .seg__btn').forEach((b) => b.classList.remove('is-active'));
 button.classList.add('is-active');
 dataMode = button.dataset.mode;
 sessionStorage.setItem('asktrix.admin.dataMode', dataMode);
 render();
 });
});

// Restore the previous choice so a refresh does not silently change what is on screen.
document.querySelectorAll('#data-mode .seg__btn').forEach((b) => {
 b.classList.toggle('is-active', b.dataset.mode === dataMode);
});
...
async function overview() {
 const s = await api(withMode('/admin/overview'));
...
function employeeTable(items) {
 ...
 <th>Calls today</th><th>Talk time</th><th>Access</th><th>Last seen</th><th>Battery</th>
 </tr></thead>
 <tbody>${items.map((e) => {
 ...
 <td class="dim" style="max-width:220px">${(e.permissions || []).length} permissions
 <div class="dim" style="font-size:10.5px;line-height:1.5">${(e.allowedStatuses || []).length} status actions</div></td>
...
async function tracking() {
 ...
 api(withMode(`/admin/employees/${selectedEmployee}/locations`)),
 api(withMode(`/admin/employees/${selectedEmployee}/attendance`)),
 ]);
...
async function calls() {
 const data = await api(withMode('/admin/calls'));
